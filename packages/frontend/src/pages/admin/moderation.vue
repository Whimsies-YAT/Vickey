	...
	<div class="_spacer" style="--MI_SPACER-w: 700px; --MI_SPACER-min: 16px; --MI_SPACER-max: 32px;">
		<SearchMarker path="/admin/moderation" :label="i18n.ts.moderation" :keywords="['moderation']" icon="ti ti-shield" :inlining="['serverRules']">
			<div class="_gaps_m">
				...
				<SearchMarker :keywords="['open', 'registration', 'signup']">
				...
				<SearchMarker :keywords="['hidden', 'user', 'abuse']">
					<MkFolder>
						<template #icon><SearchIcon><i class="ti ti-info-circle"></i></SearchIcon></template>
						<template #label><SearchLabel>{{ i18n.ts.abuseReportAutoProcessing }}</SearchLabel><span class="_beta">{{ i18n.ts.beta }}</span></template>

						<div class="_gaps">
							<MkSwitch v-model="abuseMLCheck">
								<template #label><SearchLabel>{{ i18n.ts._abuseReportAutoProcessing.enable }}</SearchLabel></template>
							</MkSwitch>
							<div v-if="abuseMLCheck">
								...
								<MkInput v-model="abuseMLInfoToken">
									<template #label><SearchLabel>{{i18n.ts._abuseReportAutoProcessing.token}}</SearchLabel></template>
								</MkInput>
...
const enableRegistration = ref(!meta.disableRegistration ?? false);
const emailRequiredForSignup = ref(meta.emailRequiredForSignup ?? false);
const ugcVisibilityForVisitor = ref(meta.ugcVisibilityForVisitor ?? 'all');
const approvalRequiredForSignup = ref(meta.approvalRequiredForSignup ?? false);
