...
import type { MiGomokuGame } from '@/models/GomokuGame.js';
import { bindThis } from '@/decorators.js';
import { IdService } from '@/core/IdService.js';
import { UserEntityService } from './UserEntityService.js';

@Injectable()
export class GomokuGameEntityService {
	constructor(
	...
	public async packDetail(
		src: MiGomokuGame['id'] | MiGomokuGame,
		hint?: {
			packedUser1?: Packed<'UserLite'>,
			packedUser2?: Packed<'UserLite'>,
		},
	): Promise<Packed<'GomokuGameDetailed'>> {
		const game = typeof src === 'object' ? src : await this.gomokuGamesRepository.findOneByOrFail({ id: src });
