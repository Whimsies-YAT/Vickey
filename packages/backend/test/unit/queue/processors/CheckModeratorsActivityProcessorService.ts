...
describe('CheckModeratorsActivityProcessorService', () => {
	...
	let emailTemplatesService: Mocked<EmailTemplatesService>;
	...
	beforeAll(async () => {
		app = await Test
			.createTestingModule({
				...
				providers: [
					...
					{
						provide: EmailTemplatesService, useFactory: () => ({ sendEmailWithTemplates: vi.fn() }),
					},
					{
		...
		emailTemplatesService = app.get(EmailTemplatesService) as Mocked<EmailTemplatesService>;
