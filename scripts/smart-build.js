...
function main() {
	...
	console.log('\x1b[1m[2/5] Building backend and all dependencies...\x1b[0m');
	console.log('[smart-build] Using pnpm workspace dependency resolution\n');
	if (!exec('pnpm --filter backend... build')) {
		console.error('\x1b[31m✗ Backend dependencies build failed\x1b[0m');
		process.exit(1);
	}
	console.log('');

	console.log('\x1b[1m[3/5] Checking and rebuilding misskey-js with types...\x1b[0m');
